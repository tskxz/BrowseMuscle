...
module.exports = {
  ...
  criar_sessao_treino_post: async (req, res) => {
    ...
    if (nome && descricao && userid) {
      ...
      let existeTreino = await SessaoTreinoService.buscarTodos_user_nome(
        userid,
        nome
      );
  ...
  // Página Utilizador - Ver as sessões de treino que o utilizador criou
  ver_sessoes_treinos: async (req, res) => {
    ...
    for (let i in SessoesTreino_user) {
      json.result.push({
        ...
        createdAt: SessoesTreino_user[i].createdAt.toLocaleDateString("pt-PT", {
          year: "numeric",
          month: "2-digit",
          day: "2-digit",
        }),
    ...
    res.render("app/SessaoTreino/lista_sessoes_treinos", {
      user: req.user,
      rows: json.result,
  ...
  ver_sessao: async (req, res) => {
    ...
    let createdAt_treino = sessaoTreino[0].createdAt.toLocaleDateString(
      "pt-PT",
    ...
    if (!sessaoTreino) {
      ...
      if (sessaoTreino_utilizador_id != userid) {
        ...
        for (let i in sessaoTreino) {
          json.result.push({
            ...
            createdAt: sessaoTreino[i].createdAt.toLocaleDateString("pt-PT", {
              year: "numeric",
              month: "2-digit",
              day: "2-digit",
            }),
      ...
      nomes_exercicios = await SessaoTreinoService.buscar_nome_exercicio(
        id_sessao
      );
    ...
    if (!sessaoTreino) {
      ...
      if (sessaoTreino_utilizador_id != userid) {
        ...
        json.result.push({
          ...
          createdAt: sessaoTreino[0].createdAt.toLocaleDateString("pt-PT", {
            year: "numeric",
            month: "2-digit",
            day: "2-digit",
          }),
  ...
  definir_sessao_treino_post: async (req, res) => {
    ...
    if (carga && reps_objetivo && exercicio_id && series) {
      let verificar_exercicio_sessaotreino =
        await SessaoTreinoService.buscarExercicioSessaoTreino(
          id_sessao,
          userid,
          exercicio_id
        );
      if (verificar_exercicio_sessaotreino.length === 0) {
        ...
        let definido = await SessaoTreinoService.definir_objetivo_exercicio(
          id_sessao,
          userid,
          exercicio_id,
          carga,
  ...
  concluir_sessao_treino_post: async (req, res) => {
    ...
    if (!sessao_concluido) {
      ...
      await SessaoTreinoService.treinoConcluidoUtilizador(
        id_user,
        treinos_concluidos
      );
  ...
  definir_reps: async (req, res) => {
    ...
    let exercicios = await SessaoTreinoService.buscarUmSessaoExercicio(
      id_sessao,
      exercicio_id
    );
    ...
    res.render("app/SessaoTreino/definir_reps", {
      user: req.user,
      rows: exercicios,
      id_sessao: id_sessao,
  ...
  definir_reps_post: async (req, res) => {
    ...
    let exercicio_concluido = await SessaoTreinoService.concluirExercicio(
      id_sessao,
      exercicio_id
    );

    // Chama o serviço para definir o número de repetições do exercício realizado
    await SessaoTreinoService.definirReps(
      reps_set1,
      reps_set2,
      reps_set3,
      reps_set4,
